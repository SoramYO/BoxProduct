...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (showAlert) {
    showAlert.classList.add("alert-show");
    let time = showAlert.getAttribute("data-time");
    time = parseInt(time);

    // After 'time' milliseconds, close the alert
    setTimeout(() => {
    ...
    // Set progress bar duration if present
    const progressBar = showAlert.querySelector(".progress-bar");
    if (progressBar) {
      progressBar.style.animationDuration = time + "ms"; // Progress bar decreases over the set time
    }
  }
});
